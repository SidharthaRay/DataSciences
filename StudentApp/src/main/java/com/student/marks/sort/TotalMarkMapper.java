...
public class TotalMarkMapper extends
	...
	public void map(LongWritable offset, Text data, Context context)
		...
		context.write(
				new Text(values.get(StudentConstants.DATA_INDEX.SCHOOL_NAME
						.getIndex())
						+ ","
						+ values.get(StudentConstants.DATA_INDEX.CLAZZ
								.getIndex())
						+ ","
						+ values.get(StudentConstants.DATA_INDEX.ROLL_NO
								.getIndex())),
				new IntWritable(Integer.parseInt(values.get(StudentConstants.DATA_INDEX.MARKS.getIndex()))));
